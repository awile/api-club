...
2. In the database window ssh into the database container:
```bash
$ docker compose exec db psql -U postgres -d api
$ api=# \dt
              List of relations
...
3. Now we can run the migration in the server window:
```bash
